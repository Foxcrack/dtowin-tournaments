// iniciar-badges.js - Script para inicializar correctamente la gestión de badges
import { auth } from './firebase.js';
import { collection, getDocs, getDoc, updateDoc, deleteDoc, doc, 
         query, where, serverTimestamp, addDoc } from "https://www.gstatic.com/firebasejs/9.19.1/firebase-firestore.js";
...
async function iniciarGestionBadges() {
    ...
    // Verificar si el usuario es admin/host
    try {
        const isHost = await esUsuarioHost();
        if (!isHost) {
    ...
    // Función para verificar si el usuario es host
    async function esUsuarioHost() {
        try {
            const usersRef = collection(db, "usuarios");
            const q = query(usersRef, where("uid", "==", user.uid), where("isHost", "==", true));
